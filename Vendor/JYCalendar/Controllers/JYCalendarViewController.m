...
@interface JYCalendarViewController () <JYCalendarMonthCellDelegate>
...
- (void)_initialize
{
    ...
    [[RACObserve(self.menuView, showed) deliverOn:RACScheduler.mainThreadScheduler] subscribeNext:^(NSNumber *showed) {
        @strongify(self);
        if (showed.boolValue) {
            [self setupNavigationBarInMenuMode];
        } else {
            [self setupNavigationBarInNormalMode];
        }
    }];
}
...
- (void)setScrollDirection:(UICollectionViewScrollDirection)scrollDirection
{
    ...
    ((UICollectionViewFlowLayout *)self.collectionViewLayout).scrollDirection = self.scrollDirection;
}
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        ...
        // Calculate where the collection view should be at the right-hand end item
        float contentOffsetWhenFullyScrolledRight = self.collectionView.frame.size.height * 2;
        
        if (scrollView.contentOffset.y == contentOffsetWhenFullyScrolledRight) {
