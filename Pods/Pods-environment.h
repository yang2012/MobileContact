
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// DAKeyboardControl
#define COCOAPODS_POD_AVAILABLE_DAKeyboardControl
#define COCOAPODS_VERSION_MAJOR_DAKeyboardControl 2
#define COCOAPODS_VERSION_MINOR_DAKeyboardControl 2
#define COCOAPODS_VERSION_PATCH_DAKeyboardControl 0

// DKLiveBlur
#define COCOAPODS_POD_AVAILABLE_DKLiveBlur
#define COCOAPODS_VERSION_MAJOR_DKLiveBlur 0
#define COCOAPODS_VERSION_MINOR_DKLiveBlur 0
#define COCOAPODS_VERSION_PATCH_DKLiveBlur 1

// FXBlurView
#define COCOAPODS_POD_AVAILABLE_FXBlurView
#define COCOAPODS_VERSION_MAJOR_FXBlurView 1
#define COCOAPODS_VERSION_MINOR_FXBlurView 3
#define COCOAPODS_VERSION_PATCH_FXBlurView 1

// JDDroppableView
#define COCOAPODS_POD_AVAILABLE_JDDroppableView
#define COCOAPODS_VERSION_MAJOR_JDDroppableView 1
#define COCOAPODS_VERSION_MINOR_JDDroppableView 1
#define COCOAPODS_VERSION_PATCH_JDDroppableView 0

// MJNIndexView
#define COCOAPODS_POD_AVAILABLE_MJNIndexView
#define COCOAPODS_VERSION_MAJOR_MJNIndexView 0
#define COCOAPODS_VERSION_MINOR_MJNIndexView 0
#define COCOAPODS_VERSION_PATCH_MJNIndexView 1

// OCMock
#define COCOAPODS_POD_AVAILABLE_OCMock
#define COCOAPODS_VERSION_MAJOR_OCMock 2
#define COCOAPODS_VERSION_MINOR_OCMock 2
#define COCOAPODS_VERSION_PATCH_OCMock 1

// OHHTTPStubs
#define COCOAPODS_POD_AVAILABLE_OHHTTPStubs
#define COCOAPODS_VERSION_MAJOR_OHHTTPStubs 2
#define COCOAPODS_VERSION_MINOR_OHHTTPStubs 0
#define COCOAPODS_VERSION_PATCH_OHHTTPStubs 0

// PonyDebugger
#define COCOAPODS_POD_AVAILABLE_PonyDebugger
#define COCOAPODS_VERSION_MAJOR_PonyDebugger 0
#define COCOAPODS_VERSION_MINOR_PonyDebugger 3
#define COCOAPODS_VERSION_PATCH_PonyDebugger 0

// SSDataKit
#define COCOAPODS_POD_AVAILABLE_SSDataKit
#define COCOAPODS_VERSION_MAJOR_SSDataKit 0
#define COCOAPODS_VERSION_MINOR_SSDataKit 1
#define COCOAPODS_VERSION_PATCH_SSDataKit 5

// SocketRocket
#define COCOAPODS_POD_AVAILABLE_SocketRocket
#define COCOAPODS_VERSION_MAJOR_SocketRocket 0
#define COCOAPODS_VERSION_MINOR_SocketRocket 3
#define COCOAPODS_VERSION_PATCH_SocketRocket 1

// TMCache
#define COCOAPODS_POD_AVAILABLE_TMCache
#define COCOAPODS_VERSION_MAJOR_TMCache 1
#define COCOAPODS_VERSION_MINOR_TMCache 2
#define COCOAPODS_VERSION_PATCH_TMCache 0

// WSCoachMarksView
#define COCOAPODS_POD_AVAILABLE_WSCoachMarksView
#define COCOAPODS_VERSION_MAJOR_WSCoachMarksView 0
#define COCOAPODS_VERSION_MINOR_WSCoachMarksView 2
#define COCOAPODS_VERSION_PATCH_WSCoachMarksView 0

